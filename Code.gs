...
function doGet(e) {
  ...
  try {
    if (action === 'getData') {
      ...
      const data = rows.slice(1).map((row, idx) => {
        ...
        return {
          ...
          _row_index: idx + 2  // simpan index baris (1-based, +1 header)
        };
    ...
    // Action: update status perbaikan
    if (action === 'updateStatus') {
      ...
      // Cari kolom Status_Perbaikan
      const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
      let statusCol = headers.indexOf('Status_Perbaikan') + 1;
...
function simpanDataSupervisi(data) {
  ...
  sheet.appendRow([
    ...
    data.status_perbaikan || 'Belum Diperbaiki'  // Status perbaikan
  ]);
